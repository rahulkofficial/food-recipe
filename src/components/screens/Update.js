  ...
  useEffect(()=>{
    ...
    async function fetchData(){
      try {
        ...
        const res=await axios.get(`${base_url}/api/v1/recipes/categories`,{
          headers:{
            Authorization:`Bearer ${accessToken}`,
          }
        });
...
const handleSubmit = async (e) => {
  e.preventDefault();
  try {
        const response=await axios.put(`${base_url}/api/v1/recipes/update/${id}`, {file:image,title:name,category:category,description:description}, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        ...
        <section className="wrapper">
            ...
            <form className="update" encType="multipart/form-data" onSubmit={handleSubmit}>
                ...
                <span className="button">
                    ...
                    <a href={`/recipe_detailed/${id}`} className="button">Cancel</a>
                </span>
            </form>
        </section>
    </section>
