...
export default function RecipesDetailed() {
  ...
  useEffect(()=>{
    ...
    async function fetchData(){
      try {
        const resp=await axios.get(`${base_url}/api/v1/recipes/details/${id}`, {
          headers: {
...
const handleUpdate=(id)=>{
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href=`${origin}/update/${id}`;
    }
  });
}
...
const handleLogout=(e)=>{
 ...
 const handleDelete=(id)=>{
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        axios.delete(`${base_url}/api/v1/recipes/delete/${id}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      }).then((response)=>{
        Swal.fire({
          ...
          window.location.href=`${origin}/recipes`;
        });
      });
  } catch (error) {
    if (error.response.status === 401) {
    ...
    <section className="all">
            <section className="wrapper">
                <header>
                    ...
                    <div>
                        <a href="/recipes" className="button">Home</a>
                        { recipe.user===user?(
                          <>
